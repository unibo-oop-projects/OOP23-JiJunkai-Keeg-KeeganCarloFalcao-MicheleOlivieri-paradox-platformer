...
public class PhysicsEngine implements Physics {
    ...
    // deltatime is ambigous, dont know whether it needs to be put as contructor
    // paramter or function neccessity
    // move paramaters are getting numerous
    @Override
    public Pair<Vector2D, Double> moveTo(Vector2D start, Vector2D end, long duration, Interpolator<Vector2D> interpType,
            long dt) {
        this.elapseTime += dt;
        double y = elapseTime / (duration * 1000.);

        // doubt, does duration express on nanosec or conventional?, needs a conversion
        // if so
    ...
    // alternatives:
    // doubts: return type of move should be void or resulting vector and so the
    // field would be
